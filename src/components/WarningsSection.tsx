const warnings = [
  { num: "01", title: "Асығыстық және қысым", text: "«Қазір шешім қабылдамасаң, кеш болады» — алаяқтар сыни ойлауды өшіру үшін уақытты шектейді. Нақты банктер мен мемлекеттік органдар ешқашан осылай сөйлемейді.", badge: "Ең жиі" },
...
const WarningsSection = () => (
  <section id="warnings" className="section-container" style={{ background: "hsl(0 0% 10%)" }}>
    <div className="max-w-7xl mx-auto">
      <div className="section-label" style={{ color: "#E57373" }}>Қалай тануға болады?</div>
      <h2 className="section-title" style={{ color: "white" }}>5 ҚЫЗЫЛ ЖАЛАУША</h2>
      <p className="section-sub" style={{ color: "#aaa" }}>
        Мына белгілердің кез келгені болса — тоқта, трубканы қой, жақыныңа хабарла.
      </p>
      ...
      <div className="border border-[#333] rounded-xl overflow-hidden">
        {warnings.map((w, i) => (
          <div
            key={w.num}
            className={`flex items-center gap-6 px-7 py-6 transition-colors hover:bg-[#252525] ${
          ...
          >
            ...
            <div className="flex-1">
              <h4 className="text-base font-bold mb-1" style={{ color: "white" }}>{w.title}</h4>
              <p className="text-[13px] leading-relaxed" style={{ color: "#888" }}>{w.text}</p>
            </div>
            <div className="ml-auto text-[11px] font-bold px-2.5 py-1 rounded-full whitespace-nowrap hidden sm:block" style={{ background: "#2a2a2a", color: "#666" }}>
              {w.badge}
            </div>
          </div>
        ))}
